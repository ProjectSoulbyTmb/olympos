...
class AppState:
    ...
    def _load(self) -> dict:
        ...
        for key in ("ratings", "tags", "positions"):
            v = raw.get(key)
...
class AphroditeHandler(BaseHTTPRequestHandler):
    ...
    WRITE_POST = ("/api/fav", "/api/rate", "/api/tag", "/api/pos", "/api/bulk")
    WRITE_DELETE = ("/api/fav", "/api/tag", "/api/pos")
    ...
    def _route(self, head: bool) -> None:
        ...
        try:
            if path in ("/", "/index.html"):
                ...
                # VLC-concept endpoint: plays EVERYTHING. Browser-safe
                # codecs redirect to /api/file (full range/seek); the
                # rest are transcoded on the fly to fragmented MP4.
                rel = q.get("f", [""])[0]
                try:
                    t0 = max(0.0, float(q.get("t", ["0"])[0] or 0))
                except ValueError:
                    self.fail(400, "bad t", head)
                    return
                try:
                    fp = self.library.checked(rel, kinds=("video",))
                except Forbidden as exc:
                    ...
                    return
                if not needs_transcode(fp, probe_media(fp)):
                    self.send_response(302)
                    self.send_header("Location",
                                     "/api/file?f=" + quote(rel))
                    self.send_header("Content-Length", "0")
                    self.end_headers()
                    return
                ffmpeg = _tool("ffmpeg.exe")
                if not ffmpeg:
                    self.fail(503, "ffmpeg not bundled", head)
                    return
                ...
                self.send_header("Cache-Control", "no-store")
                self.send_header("Connection", "close")  # EOF-delimited
                self.end_headers()
                if head:
                    return
                ...
                try:
                    ...
                    pass  # client seek/leave: stop feeding
                finally:
                    if proc.poll() is None:
                        proc.kill()
            elif path == "/api/subs":
                # sidecar subtitles: <same-name>.vtt preferred, .srt
                # converted to WebVTT on the fly.
                try:
                ...
                if vtt.is_file():
                    ...
                    body = srt_to_vtt(srt.read_text(
                        encoding="utf-8", errors="replace"))
                else:
                    self.fail(404, "no sidecar subtitles", head)
                    return
