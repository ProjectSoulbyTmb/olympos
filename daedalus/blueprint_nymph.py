...
def main():
    ...
    for rel in spec.get("compile", []):
        ...
        if not os.path.isfile(full):
            ...
            continue
        # syntax-only probe: compile() writes no bytecode, so it is
        # portable - py_compile(cfile=os.devnull) dies on Windows,
        # where py_compile refuses to treat "nul" as a regular file
        try:
            with open(full, "rb") as src_fh:
                compile(src_fh.read(), full, "exec")
        except SyntaxError as exc:
            emit(signature="compile-break", target=rel,
                 detail=f"{rel}: line {exc.lineno}: {exc.msg}"[:400],
                 severity="T1")
        except (ValueError, OSError) as exc:
            emit(signature="compile-break", target=rel,
                 detail=str(exc)[:400], severity="T1")
