...
def tiny_bmp() -> bytes:
    """A valid 2x2 24bpp uncompressed BMP (v0.5 normalization fixture)."""
    w, h = 2, 2
...
def make_fixture(tmp: str) -> str:
    ...
    w(os.path.join("nested", "d.bmp"), tiny_bmp())   # v0.5: needs norm
...
class Session:
    ...
    def close(self):
        ...
        shutil.rmtree(base / "APHRODITE" / "normalized" / key,
                      ignore_errors=True)
...
def t_all_recursive():
    ...
    assert paths == {"photos/a.jpg", "photos/a-copy.jpg",
                     "videos/b.mp4", "nested/c.png",
                     "nested/d.bmp"}, paths          # + v0.5 norm fixture
    assert d["total"] == 5 and d["truncated"] is False
    return "5 media, none hidden"
...
# ---- v0.5: normal-format findings + normalized serving --------------------
def t_findings_carry_fmt_std():
    st, j = S.jget("/api/tree?dir=nested")
    by = {f["name"]: f for f in j["data"]["files"]}
    assert by["c.png"]["std"] is True and by["c.png"]["fmt"] == "png", by
    assert by["d.bmp"]["std"] is False and by["d.bmp"]["fmt"] == "bmp", by
    return "findings speak fmt/std"


def t_norm_bmp_served_as_jpeg():
    st, h, body = S.raw("GET", "/api/file?f=nested/d.bmp&norm=1")
    assert st == 200, st
    assert body[:2] == b"\xff\xd8", f"expected JPEG got {body[:4]!r}"
    assert h.get("Content-Type") == "image/jpeg", h.get("Content-Type")
    return f"BMP -> JPEG ({len(body)} bytes)"


def t_norm_std_passthrough_exact():
    st, _, body = S.raw("GET", "/api/file?f=nested/c.png&norm=1")
    assert st == 200 and body == PNG_1PX, "std file must not be re-encoded"
    return "png norm view byte-identical"


def t_norm_containment():
    st, j = S.jget("/api/file?f=../secret.txt&norm=1")
    assert st == 403, (st, j)
    st2, _ = S.jget("/api/file?f=%2e%2e%2fsecret.txt&norm=1")
    assert st2 == 403, st2
    st3, _ = S.jget("/api/file?f=nested/notes.txt&norm=1")
    assert st3 == 403, st3
    return "norm route confined like the rest"


def t_library_pristine_after_norm():
    p = Path(S.root) / "nested" / "d.bmp"
    assert p.read_bytes().startswith(b"BM"), "media root was mutated!"
    return "library untouched"
...
def main() -> int:
    ...
    try:
        ...
        check("norm=1 serves BMP as JPEG", t_norm_bmp_served_as_jpeg)
        check("norm=1 std passthrough exact", t_norm_std_passthrough_exact)
        check("norm route containment", t_norm_containment)
        check("library pristine after norm", t_library_pristine_after_norm)
    finally:
        S.close()
