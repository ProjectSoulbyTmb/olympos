...
- 2026-08-24: POSEIDON learned to mend its own wounds. New `poseidon/heal.py`: every automated cycle now opens with a light self-repair pass - corrupt/missing writer berths rebuilt (Windows read-only `.git` stubs handled), orphaned throwaway indexes swept after an age floor, torn JSONL ledger tails trimmed (mid-file rot always left for an operator), and a quarantine earned by dead infrastructure is probed each cycle and released early when fetch/lane/berth probes run green (the breaker itself stays honest: persistent causes re-trip it). Each push carries auto-fixes: non-fast-forward rejections adopt origin's copy of the single-writer branch, replay the snapshot cargo (cherry-pick first, per-path overlay fallback so harmless adopted-side files survive) and push again as a fast-forward - never force-pushed; transient network/gh errors retry once on a backoff (`POSEIDON_RETRY_BACKOFF_S`). Real main-vs-drift cherry-pick conflicts still fail into the quarantine breaker untouched. Operator surface: `python -m poseidon heal [--apply] [--deep]` with object-database fsck; findings report mirror doctrine violations (local-only commits on main) without ever resetting them. Gate grew to 18 checks: `python poseidon/verify_poseidon.py`.
