...
going); supersedes the proposed `fleet.json` - `realms/registry.json`
already landed on `origin/main` and is extended here instead.

Target lineage: `origin/main @ d9aaa83`. Local commits `4a17caf`
(game-sim removal) and `d70c73f` (site hub) conflict with the declared
goal below and are handled in §9 (Reconciliation).
