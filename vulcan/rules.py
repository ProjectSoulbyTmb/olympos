...
class RuleEngine:
    ...
    def add_rule(self, spec):
        ...
        problems = validate_rule(spec)
        if problems:
            raise ValueError("rule rejected: " + "; ".join(problems[:4]))
        rule = dict(spec)
    ...
    def import_state(self, state):
        ...
        RIG-schema: invalid rules are quarantined (disabled + reason)
        instead of poisoning the engine; only structural payload errors
        raise."""
        from schema import validate_rule
        ...
        for r in rules:
            ...
            if problems:
                rule["enabled"] = False
                rule["quarantine_reason"] = "; ".join(problems[:3])
                quarantined.append((rid, rule["quarantine_reason"]))
            restored.append(rule)
        self.rules = restored
        for rid, why in quarantined:
            try:
                self.world.alert("warn",
                                 f"rule '{rid}' quarantined on load: "
                                 f"{why}")
            except Exception:
                pass
