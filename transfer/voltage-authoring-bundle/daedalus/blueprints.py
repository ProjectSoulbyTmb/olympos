...
try:
    from daedalus.blueprint_uishell import BLUEPRINT as _UI_BP
    BLUEPRINTS["ui-shell"] = dict(_UI_BP)
except ImportError:  # pragma: no cover - optional target
    pass

try:
    from daedalus.blueprint_apollo import BLUEPRINT as _APOLLO_BP
    BLUEPRINTS["apollo-os"] = dict(_APOLLO_BP)
except ImportError:  # pragma: no cover - optional target
    pass

for _mod_name, _bp_name in (
        ("blueprint_kinemahost", "kinema-host"),
        ("blueprint_rileybridge", "riley-bridge"),
        ("blueprint_medialane", "media-lane"),
        ("blueprint_entcomposer", "ent-composer"),
        ("blueprint_gamedomain", "game-domain"),
        ("blueprint_knowgateway", "know-gateway"),
        ("blueprint_learngateway", "learn-gateway"),
        ("blueprint_muse", "muse-curriculum"),
        ("blueprint_voltagetasks", "voltage-tasks"),
        ("blueprint_opsdomain", "ops-domain"),
        ("blueprint_sealchain", "session-seal"),
        ("blueprint_slapulse", "sla-pulse"),
        ("blueprint_packager", "voltage-packager"),
        ("blueprint_ares", "ares-vault")):
    try:
        _module = __import__("daedalus." + _mod_name,
                             fromlist=["BLUEPRINT"])
        BLUEPRINTS[_bp_name] = dict(_module.BLUEPRINT)
    except ImportError:  # pragma: no cover - optional target
        pass
