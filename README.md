...
| `daedalus/` | **Workshop**: autonomous server builder over its ATLAS subfleet - blueprint designs woven into guest worlds, self-test gates run inside the jail, fault-injected builds converge via fix passes (verify-fix-retry), VULCAN-style schema gates + policy rules + warden self-healing (stuck lanes, failure storms quarantined), sealed+hashed artifacts on `127.0.0.1:43905`. Planning station (2026-08-25): durable operator-signed work orders (`python -m daedalus plan ...`, draft->approved->commissioned->done, file-backed + restart-safe) feeding build steps straight into the workshop pump; 8-lane resizable subfleet (`DAEDALUS_LANES` env, `fleet resize N`). Verify: `python daedalus/verify_daedalus.py` |
